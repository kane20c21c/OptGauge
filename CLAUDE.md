...
사람이 읽을 문서(리포트·분석·의사결정 기록)는 옵시디언 볼트로 내보낸다. 데이터 산출물
(parquet·JSON·로그·캐시)은 대상이 아니다 — 저장소 안에만 둔다.

- **규칙 정본은 `StoLab/CLAUDE.md` (= `StoLab-meta/CLAUDE.md`) 의 "산출물의 옵시디언 포팅"
  절 하나뿐이다.** 볼트 경로·파일명·프론트매터·링크 규칙을 이 파일에 복사하지 말고 그 절을
  읽을 것 (드리프트 방지).
- 이 저장소의 대상 폴더: `$OBSVAULT_PATH/10_StockLaboratory/OptGauge/`
- 정본을 읽을 수 없는 환경(우산 폴더 없는 단독 클론 등)이면 **포팅하지 말고 케인에게 묻는다.**
